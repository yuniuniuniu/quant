...
typedef struct _MdsL2StockSnapshotBody {
    ...
    union {
        ...
        struct {
            int32       BondWeightedAvgPx;      /**< 债券加权平均价 (适用于质押式回购及债券现券交易产品, 表示质押式回购成交量加权平均利率及债券现券交易成交量加权平均价) */
            int32       BondAuctionTradePx;     /**< 深交所债券匹配成交的最近成交价 (仅适用于深交所债券现券交易产品. 价格单位精确到元后四位, 即: 1元=10000) */
            uint64      BondAuctionVolumeTraded;/**< 深交所债券匹配成交的成交总量 (仅适用于深交所债券现券交易产品) */
        };
    };
